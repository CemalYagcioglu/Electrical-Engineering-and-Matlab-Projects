 qfunction PhoneNum = dtmfdialUnknown(KeyNames, fs)
    ...
    for i=1:size(KeyNames,2)
        ...
        for(c=1:3)
            cf = dtmf.colTones(1,c);
            subplot(7,1,c)
            [acor,lag]=xcorr(UnknownKey,sin(2*pi*cf*t1));
            plot(lag,acor)
            %display(freqString(1))
            s = strcat('Signal Unknown vs Frequency: ',freqString(c),' Correlation');
            display(cf)
            title(s);
        end
        for(r=1:4)
            ...
            %display(kf)
            subplot(7,1,3+r)
            [acor,lag]=xcorr(UnknownKey,sin(2*pi*kf*t1));
            plot(lag,acor)
            %display(freqString)
            s = strcat('Signal Unknown vs Frequency: ',freqString(r+3),' Correlation');
            title(s);
